...
export class TextFile extends BaseFile {
  ...
  lines() {
    ...
    async function readNext(accessHandle: OPFSWorkerAccessHandle) {
      if (cacheStr.length === 0 && readDoned)
  ...
  async append(str: string) {
    await this.initReady;
    await this.accessHandle?.write(this.#txtEC.encode(str).buffer, {
  ...
  async text() {
    const txtDC = new TextDecoder();
    return txtDC.decode(await this.accessHandle?.read(0, await this.getSize()));
  }
}
